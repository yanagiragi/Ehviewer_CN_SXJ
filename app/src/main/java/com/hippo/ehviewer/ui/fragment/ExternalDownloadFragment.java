...
public class ExternalDownloadFragment extends BasePreferenceFragmentCompat implements
    ...
    public void onCreatePreferences(@Nullable Bundle savedInstanceState, @Nullable String rootKey) {
        addPreferencesFromResource(R.xml.external_download_settings);
    ...
    public void onUpdateDownloadLocation() {
        UniFile file = Settings.getExternalDownloadLocation();
        if (mDownloadLocation != null) {
            if (file != null) {
                mDownloadLocation.setSummary(file.getUri().toString());
            } else {
                mDownloadLocation.setSummary(R.string.settings_external_download_invalid_download_location);
            }
        }
    }
    ...
    private void showDirPickerDialogKK() {
        new AlertDialog.Builder(getActivity()).setMessage(R.string.settings_external_download_pick_dir_kk)
                .setPositiveButton(R.string.settings_external_download_continue, (dialog, which) -> openDirPicker()).show();
    }
    ...
    private void showDirPickerDialogL() {
        ...
        new AlertDialog.Builder(getActivity()).setMessage(R.string.settings_external_download_pick_dir_l)
                .setPositiveButton(R.string.settings_external_download_continue, listener)
                .setNeutralButton(R.string.settings_external_download_document, listener)
                .show();
    }

    private void openDirPicker() {
        UniFile uniFile = Settings.getExternalDownloadLocation();
        ...
        switch (requestCode) {
            case REQUEST_CODE_PICK_IMAGE_DIR: {
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    if (uniFile != null) {
                        Settings.putExternalDownloadLocation(uniFile);
                        onUpdateDownloadLocation();
                    } else {
                        Toast.makeText(getActivity(), R.string.settings_external_download_cant_get_download_location,
